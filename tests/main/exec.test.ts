...
describe("RunTest", () => {
  test.only("popcnt", async () => {
    ...
    const apiInst = await WebAssembly.instantiate(buffer).then(
  ...
  test("simpletest", async () => {
    ...
    const inst = await WMTS.WebAssemblyMts.instantiate(buffer).then(
      (res) => res.instance.exports,
    );
  ...
  test("block", async () => {
    ...
    const tmodule = await WebAssembly.instantiate(buffer).then(
      (res) => res.instance.exports,
    );
    const inst = await WMTS.WebAssemblyMts.instantiate(buffer).then(
      (res) => res.instance.exports,
  ...
  test("arrays", async () => {
    ...
    const inst = await WMTS.WebAssemblyMts.instantiate(buffer, {
      imports: { reduce_func: (x: number, y: number) => x + y },
    });
  ...
  test("tablesec.wasm", async () => {
    const buffer = new Uint8Array(
      fs.readFileSync("./tests/wasm/tablesec.wasm"),
    );
  ...
  test("multitablesecOffset.wasm", async () => {
    const buffer = new Uint8Array(
      fs.readFileSync("./tests/wasm/multitablesecOffset.wasm"),
    );
...
describe("Javascript WebAssembly namespace tests", () => {
  ...
  test("true-false with integers, parameter", async () => {
    const buffer = new Uint8Array(
      fs.readFileSync("./tests/wasm/truefalsevar.wasm"),
    );
    const tmodule = await WebAssembly.instantiate(buffer).then(
      (res) => res.instance.exports,
    );
