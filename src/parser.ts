...
export class CustomSection extends Section<types.custom> {
  constructor(
    public id: types.WASMSectionID.WACustom,
    public size: number,
    public content: types.custom[],
  ) {
...
export class TypeSection extends Section<types.funcType> {
  constructor(
    public id: types.WASMSectionID.WAType,
    public size: number,
    public content: types.funcType[],
  ) {
    super(id, size, content);
  }
}
export class ImportSection extends Section<types.imports> {
  constructor(
    public id: types.WASMSectionID.WAImport,
    public size: number,
    public content: types.imports[],
  ) {
    super(id, size, content);
  }
}
export class FunctionSection extends Section<number> {
  //number array (vector of type indices)
  constructor(
    public id: types.WASMSectionID.WAFunction,
    public size: number,
...
export class TableSection extends Section<types.tableType> {
  constructor(
    public id: types.WASMSectionID.WATable,
    public size: number,
    public content: types.tableType[],
  ) {
    super(id, size, content);
  }
}
export class MemorySection extends Section<types.limits> {
  constructor(
    public id: types.WASMSectionID.WAMemory,
    public size: number,
    public content: types.limits[],
  ) {
    super(id, size, content);
  }
}
export class GlobalSection extends Section<types.global> {
  constructor(
    public id: types.WASMSectionID.WAGlobal,
    public size: number,
    public content: types.global[],
  ) {
    super(id, size, content);
  }
}
export class ExportSection extends Section<types.exports> {
  constructor(
    public id: types.WASMSectionID.WAExport,
    public size: number,
    public content: types.exports[],
  ) {
    super(id, size, content);
  }
}
export class StartSection extends Section<number> {
  constructor(
    public id: types.WASMSectionID.WAStart,
    public size: number,
...
export class ElementSection extends Section<types.elem> {
  constructor(
    public id: types.WASMSectionID.WAElement,
    public size: number,
    public content: types.elem[],
  ) {
    super(id, size, content);
  }
}
export class CodeSection extends Section<types.code> {
  constructor(
    public id: types.WASMSectionID.WACode,
    public size: number,
    public content: types.code[],
  ) {
    super(id, size, content);
  }
}
export class DataSection extends Section<types.data> {
  constructor(
    public id: types.WASMSectionID.WAData,
    public size: number,
    public content: types.data[],
  ) {
    super(id, size, content);
  }
}
export class DataCountSection extends Section<number> {
  constructor(
    public id: types.WASMSectionID.WADataCount,
    public size: number,
    public content: number | null,
  ) {
    super(id, size, content);
...
export function parseSection(
  bytes: Uint8Array,
  index: number,
): [section: Section<any> | Object, index: number] {
  ...
  switch (
    sectionId // passing index+width so it skips the section id and the size (size can be between 1 and 4 bytes)
  ) {
    case WASMSectionID.WACustom:
      return [
        new CustomSection(
          sectionId,
          secSize,
          bp.parseCustom(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAType:
      return [
        new TypeSection(sectionId, secSize, bp.parseType(bytes, index + width)),
        indexAfterSection,
      ];
    case WASMSectionID.WAImport:
      return [
        new ImportSection(
          sectionId,
          secSize,
          bp.parseImport(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAFunction:
      return [
        new FunctionSection(
          sectionId,
          secSize,
          bp.parseFunction(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WATable:
      return [
        new TableSection(
          sectionId,
          secSize,
          bp.parseTable(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAMemory:
      return [
        new MemorySection(
          sectionId,
          secSize,
          bp.parseMemory(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAGlobal:
      return [
        new GlobalSection(
          sectionId,
          secSize,
          bp.parseGlobal(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAExport:
      return [
        new ExportSection(
          sectionId,
          secSize,
          bp.parseExport(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WAStart: {
      if (secSize == 0)
        return [new StartSection(sectionId, secSize, null), width + index]; // no content inside section Start
    ...
    case WASMSectionID.WAElement:
      return [
        new ElementSection(
          sectionId,
          secSize,
          bp.parseElement(bytes, index + width),
        ),
        indexAfterSection,
      ];
    case WASMSectionID.WACode:
      return [
        new CodeSection(sectionId, secSize, bp.parseCode(bytes, index + width)),
        indexAfterSection,
      ];
    case WASMSectionID.WAData:
      return [
        new DataSection(sectionId, secSize, bp.parseData(bytes, index + width)),
        indexAfterSection,
      ];
    case WASMSectionID.WADataCount: {
      if (secSize == 0)
        return [new DataCountSection(sectionId, secSize, null), width + index];
      const [integer, idxwidth] = helperParser.parseidx(bytes, index + width);
      return [
        new DataCountSection(sectionId, secSize, integer),
        indexAfterSection,
      ];
    }
    default:
      throw new Error(`Invalid secID "${sectionId}".`);
  }
}
