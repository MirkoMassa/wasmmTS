...
export function checkDualArityFn(x: Op, y: Op, opcode: Opcode) {
  if (x.id != opcode || y.id != opcode)
    throw new Error(
      `Invalid number types expect (${Opcode[opcode]}, ${
        Opcode[opcode]
      }) got [${Opcode[x.id]},${Opcode[y.id]}]`,
    );
}
export function checkTypeOpcode(x: Op, opcode: Opcode) {
  if (x.id != opcode)
    throw new Error(
      `Invalid number types expect (${Opcode[opcode]}) got [${Opcode[x.id]}]`,
    );
}
...
export function isNumber(x: Op) {
  switch (x.id) {
    ...
    default:
      throw new Error(
        `Invalid value: expected a numeric val, got [${Opcode[x.id]}]`,
      );
  }
}
...
export function checkValTypeAndOp(x: Op, y: valType) {
  ...
  if (x.id != valToOp)
    throw new Error(
      `Invalid valType expect (${ValTypeEnum[y]}) got [${Opcode[x.id]}]`,
    );
}
export function convertValTypeToOpCode(vt: valType): Opcode {
  switch (vt) {
    ...
    case 0x70:
      return Opcode.RefFunc;
    // case 0x6f: return //externref
    default:
      throw new Error("Unexpected valType");
  }
}
...
function isFuncRef(x: unknown): x is FuncRef {
  // @ts-ignore
  return (
    typeof x == "object" && x != null && x.type !== undefined && x.type === 0x70
  );
}
...
export function processParams(
  ...
  if (arity < args.length)
    throw new Error(
      `Unexpected number of parameters. Got ${args.length}, expected ${arity}.`,
    );
  for (let i = 0; i < args.length; i++) {
    ...
    if (typeof args[i] == "number") {
      ...
      switch (types[i]) {
        ...
        case "f64":
          //All ok
          locals[i].value = currentArg as number;
          break;
        default:
          throw new Error();
      }
      ...
      if (types[i] != "funcref" && typeof types[i] !== "function")
        throw new Error();
      // idk @todo
...
export function i32ltu(x: Op, y: Op) {
  checkDualArityFn(x, y, Opcode.I32Const);
  if (Math.abs(y.args as number) < Math.abs(x.args as number))
    return new Op(Opcode.I32Const, 1);
  return new Op(Opcode.I32Const, 0);
}
export function i64ltu(x: Op, y: Op) {
  checkDualArityFn(x, y, Opcode.I64Const);
  if (Math.abs(y.args as number) < Math.abs(x.args as number))
    return new Op(Opcode.I64Const, 1);
  return new Op(Opcode.I64Const, 0);
}
...
export function f32gt(x: Op, y: Op) {
  checkDualArityFn(x, y, Opcode.F32Const);
  if (Math.abs(y.args as number) > (x.args as number))
    return new Op(Opcode.F32Const, 1);
  return new Op(Opcode.F32Const, 0);
}
export function f64gt(x: Op, y: Op) {
  checkDualArityFn(x, y, Opcode.F64Const);
  if (Math.abs(y.args as number) > (x.args as number))
    return new Op(Opcode.F64Const, 1);
  return new Op(Opcode.F64Const, 0);
}
//le
...
export function executeBlock(
  ...
  if (isValType(block.bt)) {
    // no params if single valtype

    label = new Label(1, block.expr, block.bt as valType);
  } else if (block.bt == 0x40) {
    // empty type

    label = new Label(0, block.expr, undefined);
  } else {
    // will have multiple returns and/or at least one param
    let paramValues: Op[] = [];
    ...
    label = new Label(
      moduleTypes[block.bt].returns.length,
      block.expr,
      moduleTypes[block.bt],
      0,
      false,
      paramValues,
    );
...
export function br(stack: Op[], labelidx: number): Label {
  if (labelCount(stack) < labelidx + 1)
    throw new Error("Not enough labels in the stack.");
  ...
  if (currLoopLabel.isblock) {
    ...
    if (
      currLoopLabel.type instanceof WasmFuncType &&
      currLoopLabel.type.parameters.length > 0
    ) {
...
export function load(
  ...
  if (N == 64) {
    ...
    for (let i = 0; i < N / 8; i++) {
      result =
        result | (BigInt(memData[i + resOffsetAddress]) << (8n * BigInt(i)));
    }
  } else {
    result = 0;
    for (let i = 0; i < N / 8; i++) {
