...
import { ExportInst, WebAssemblyMtsInstantiatedSource, storeProducePatches } from "./types";
import WebAssemblyMts from "./wasmm";
import { valType } from "../types";


export function createExports(exportInstances:ExportInst[], instantiatedSource:WebAssemblyMtsInstantiatedSource){
...
function exportFunctions(exp:ExportInst, instantiatedSource:WebAssemblyMtsInstantiatedSource) {
        ...
        instantiatedSource.instance.exportsTT[exp.valName] = (...args: any[]) => {
            const funcRes:{val: Op | Op[], stores: storeProducePatches} = WebAssemblyMts.runTT(exp, ...args);
            return funcRes;
        }
