    ...
    if (rawType.parameters != undefined) {
      for (let i = 0; i < rawType.parameters[0]; i++) {
        switch (rawType.parameters[1][i]) {
          case 0x7f:
            this.parameters.push("i32");
            break;
          case 0x7e:
            this.parameters.push("i64");
            break;
          case 0x7d:
            this.parameters.push("f32");
            break;
          case 0x7c:
            this.parameters.push("f64");
            break;
          case 0x70:
            this.parameters.push("funcref");
            break;
          case 0x6f:
            this.parameters.push("externref");
            break;
          case 0x7b:
            this.parameters.push("vectype");
            break;
          default:
    ...
    if (rawType.returns != undefined) {
      for (let i = 0; i < rawType.returns[0]; i++) {
        switch (rawType.returns[1][i]) {
          case 0x7f:
            this.returns.push("i32");
            break;
          case 0x7e:
            this.returns.push("i64");
            break;
          case 0x7d:
            this.returns.push("f32");
            break;
          case 0x7c:
            this.returns.push("f64");
            break;
          case 0x70:
            this.returns.push("funcref");
            break;
          case 0x6f:
            this.returns.push("externref");
            break;
          case 0x7b:
            this.returns.push("vectype");
            break;
          default:
...
export function instantiateParams(
  parameters: WasmType[],
): parserTypes.localsVal[] {
  const params: parserTypes.localsVal[] = [];
  for (let i = 0; i < parameters.length; i++) {
    switch (parameters[i]) {
      case "i32":
        params[i] = { value: 0, type: 0x7f };
        break;
      case "i64":
        params[i] = { value: 0n, type: 0x7e };
        break;
      case "f32":
        params[i] = { value: 0, type: 0x7d };
        break;
      case "f64":
        params[i] = { value: 0, type: 0x7c };
        break;
      case "funcref":
        params[i] = { value: 0, type: 0x70 };
        break;
      case "externref":
        params[i] = { value: 0, type: 0x6f };
        break;
      case "vectype":
        params[i] = { value: 0, type: 0x7b };
        break;
      default:
        throw new Error("Invalid return type");
    }
...
export class WebAssemblyMtsStore implements types.Store {
  ...
  public stack: Op[];
  constructor(
    public funcs: types.FuncInst[] = [],
    public tables: types.TableInst[] = [],
    public mems: types.MemInst[] = [],
    public globals: types.GlobalInst[] = [],
    public elems: types.ElemInst[] = [],
    public exports: types.ExportInst[] = [],
  ) {
    this.stack = [];
  }
  ...
  executeOp(op: Op | IfElseOp, currLabel: Label): void | Op[] | Label {
    ...
    switch (op.id) {
      ...
      case Opcode.Block: {
        ...
        const labelRes = execute.executeBlock(
          op as BlockOp,
          moduleTypes,
          this.stack,
        )!;
      ...
      case Opcode.Return: {
        ...
        if (this.stack.length < returnArity)
        ...
        do {
          ...
          !(this.stack[this.stack.length - 1] instanceof Frame) ||
          this.stack.length == 0
        );
      ...
      case Opcode.Call: {
        const frame = lookForFrame(this.stack);
        if (frame?.module.funcs[op.args as number] == undefined)
          throw new Error(`Function (typeidx ${op.args} doesn't exists.`);
      ...
      case Opcode.CallIndirect: {
        ...
        if (table == undefined)
          throw new Error(`No table found at tableidx '${tableidx}'`);
        const tableAddr = this.stack.pop()!;
        if (tableAddr.id != Opcode.I32Const)
          throw new Error(
            `Invalid table address, expected I32Const, got ${tableAddr.kind}`,
          );
      ...
      case Opcode.RefIsNull: {
        ...
        if (val?.id != Opcode.RefNull && val?.id != Opcode.RefFunc) {
          throw new Error(
            `No reference value on top of the stack, instead got '${val?.kind}'.`,
          );
        }
      ...
      case Opcode.RefFunc: {
        ...
        if (this.funcs[funcidx] == undefined)
          throw new Error(`Undefined function (funcidx '${funcidx}').`);
        this.stack.push(op);
        break;
      }
      // Vector Instructions
      ...
      case Opcode.SetLocal: {
        const frame = lookForFrame(this.stack);
        const localToSet: parserTypes.localsVal =
          frame!.locals[op.args as number];
        execute.setLocal(localToSet, this.stack.pop()!);
        break;
      }
      case Opcode.GetLocal: {
        const frame = lookForFrame(this.stack);
        const localToGet: parserTypes.localsVal =
          frame!.locals[op.args as number];
      ...
      case Opcode.TeeLocal: {
        // like set but keeping the const
        const frame = lookForFrame(this.stack);
      ...
      case Opcode.i32sub: {
        ...
        if (this.stack.length < 2)
          throw new Error(`Expecting 2 arguments, got ${this.stack.length}`);
      ...
      case Opcode.i32remU: {
        let [divisorOp, dividendOp] = constParamsOperationValues(
          this.stack,
          currLabel,
        );
...
export class WebAssemblyMts {
  ...
  static async compile(
    bytes: Uint8Array,
    importObject: Object | undefined,
  ): Promise<[types.WebAssemblyMtsModule, object]> {
    ...
    let customSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WACustom,
    )?.content;

    let typesSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAType,
    );
    let importSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAImport,
    );
    let functionSignatures = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAFunction,
    );
    let codeSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WACode,
    );
    let tableSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WATable,
    );
    let memorySection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAMemory,
    );
    let globalSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAGlobal,
    );
    let elemSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAElement,
    );
    let dataSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAData,
    );
    let exportSection = moduleTree.sections.find(
      (sec) => sec.id == parserTypes.WASMSectionID.WAExport,
    );
    // Types
    ...
    if (importSection != undefined) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        mtsModule.funcs.push({ kind: "funcaddr", val: imports[i].description });
      }
    }
    ...
    for (let i = imports.length; i < codes.length; i++) {
      ...
      WebAssemblyMts.store.funcs.push(func);
      mtsModule.funcs.push({ kind: "funcaddr", val: currTypeSignature });
    ...
    for (let index in elemSection?.content) {
      ...
      if (currElem.activemode != undefined)
        tableID = currElem.activemode.table as number;
      ...
      mtsModule.elems.push({ kind: "elemaddr", val: length - 1 });
    }
    // TableInst
    ...
    for (let index in tableSection?.content) {
      ...
      const tableSize = limFlag
        ? (currTable.max as number)
        : (currTable.min as number);
      ...
      mtsModule.tables.push({ kind: "tableaddr", val: len - 1 });
    }
    ...
    for (let index in memorySection?.content) {
      ...
      mtsModule.mems.push({ kind: "memaddr", val: length - 1 });
    }
    // GlobalInst
    for (let index in globalSection?.content) {
      ...
      mtsModule.globals.push({ kind: "globaladdr", val: length - 1 });
    }
  ...
  static async instantiate(
    moduleMts: types.WebAssemblyMtsModule,
    importObject?: object,
  ): Promise<types.WebAssemblyMtsInstance>;
  static async instantiate(
    bytes: Uint8Array,
    importObject?: object,
  ): Promise<types.WebAssemblyMtsInstantiatedSource>;

  static async instantiate(
    moduleOrBytes: unknown,
    importObject?: object,
  ): Promise<
    types.WebAssemblyMtsInstantiatedSource | types.WebAssemblyMtsInstance
  > {
    if (moduleOrBytes instanceof Uint8Array) {
      ...
      const instantiatedSource: types.WebAssemblyMtsInstantiatedSource = {
        ...
        instance: { exports: {}, exportsTT: {}, object: undefined, custom: {} },
      };
      moduleOrBytes.exports.forEach((exp) => {
        if (exp.value.kind == "funcaddr") {
          ...
          // time travel
          instantiatedSource.instance.exportsTT[exp.valName] = (
            ...args: any
          ) => {
  ...
  static run(func: unknown, ...args: unknown[]) {
    ...
    label = new Label(
      funcInstance!.type.returns.length,
      funcInstance!.code.body,
      funcInstance!.type,
    );
    // activation frame (locals and module)
    const params: parserTypes.localsVal[] = instantiateParams(
      funcInstance!.type.parameters,
    );
    const locals: parserTypes.localsVal[] = instantiateLocals(
      funcInstance!.code.locals,
    );
  ...
  static executeInstructions(label: Label, returnsArity: number): Op | Op[] {
    ...
    try {
      while (lookForLabel(this.store.stack) != undefined) {
        ...
        if (currLabel.instrIndex < currLabel.instr.length) {
          ...
          const res = this.store.executeOp(
            currLabel.instr[currLabel.instrIndex],
            currLabel,
          );
          if (res instanceof Label) {
            // changing label pointer mainly for br/brif
            currLabel = res;
          }
          ...
          if (this.store.stack[this.store.stack.length - 1] != currLabel)
            throw new Error("No label on top of the stack.");
    ...
    if (this.store.stack.length < returnsArity)
      throw new Error("Not enough return elements in the stack");
    if (returnsArity > 1) {
  ...
  static executeInstructionsTT(
    returnsArity: number,
    stores: types.storeProducePatches,
  ): { stores: types.storeProducePatches; val: Op | Op[] } {
      ...
      while (lookForLabel(currStore.stack) != undefined) {
        ...
        const produced = produceWithPatches(currStore, (state) => {
          ...
          if (currLabel.instrIndex < currLabel.instr.length) {
            ...
            if (state.stack[state.stack.length - 1] != currLabel)
              throw new Error("No label on top of the stack.");
            state.stack.pop();
            labelRes.forEach((val) => {
    ...
    if (currStore.stack.length < returnsArity)
      throw new Error("Not enough return elements in the stack");
    if (returnsArity > 1) {
      const res = new Array(returnsArity);
      for (let i = returnsArity - 1; i >= 0; i--) {
...
// helper funcs
export function isWebAssemblyModule(
  data: unknown,
): data is types.WebAssemblyMtsModule {
...
export function constParamsOperationValues(
  stack: Op[],
  currLabel: Label,
): [Op, Op] {
...
export type MaskedArrayObject = Map<string | symbol | number, number> & {
  [key: string]: number;
};
export function createMemProxy(): MaskedArrayObject {
  ...
  return new Proxy(wrappedBackingMap, {
    get(target, property, receiver) {
      if (property == "forEach") {
        return <T, K>(
          cbFunction: (value: T, key: K, map: Map<K, T>) => void,
        ) => {
