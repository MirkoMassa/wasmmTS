...
export function buildStateStrings(
  stores: storeProducePatches,
  customSec: custom[],
): stateDescriptor[] {
  ...
  for (let i = 0; i < stores.states.length; i++) {
    ...
    for (let j = currStore.stack.length - 1; j >= 0; j--) {
      ...
      const [type, description] = stringBuilder(
        elem,
        currStore,
        moduleCustom,
        funcsCustom,
...
export function descCurrentLabel(
  currStore: WebAssemblyMtsStore,
  currLabel: Label | undefined = undefined,
): string {
...
export function descCurrentFrame(
  currStore: WebAssemblyMtsStore,
  currFrame: Frame | undefined = undefined,
  funcsCustom: nameAssoc[],
  localsCustom: indirectNameAssoc[],
): string {
  ...
  if (currFrame != undefined) {
    ...
    for (let j = 0; j < currFrame.locals.length; j++) {
      ...
      localsStringified = localsStringified.concat(
        `'${localName}'= ${currFrame.locals[j].value} (${localType}), `,
      );
    }
    localsStringified = localsStringified
      .slice(0, localsStringified.length - 2)
      .concat(".");
...
export function getCustoms(
  customSec: custom[],
): [namesVector, nameAssoc[], indirectNameAssoc[]] {
  const moduleCustom = customSec.find((subsec) => subsec.subsecId == 0)
    ?.names as namesVector;
  const funcsCustom = customSec.find((subsec) => subsec.subsecId == 1)
    ?.names as nameAssoc[];
  const localsCustom = customSec.find((subsec) => subsec.subsecId == 2)
    ?.names as indirectNameAssoc[];
...
export function stringBuilder(
  elem: Op,
  currStore: WebAssemblyMtsStore,
  moduleCustom: namesVector,
  funcsCustom: nameAssoc[],
  localsCustom: indirectNameAssoc[],
): [descriptionTypes, string] {
  let type: descriptionTypes,
    description = "";
      ...
      default: {
        ...
        if (elem.kind.includes("Local")) {
          ...
          const localName: string =
            localsCustom[funcidx][1][elem.args as number][1][1];
...
export function buildPatchesStrings(
  stores: storeProducePatches,
  customSec: custom[],
): patchesDescriptor[] {
  const [moduleCustom, funcsCustom, localsCustom] = getCustoms(customSec);
  ...
  stores.patches.forEach((patch, i) => {
    ...
    patch.forEach((operation, j) => {
      ...
      } else if (operation.value instanceof Op) {
        // constants
      ...
      if (operation.op == "replace") {
        operations.push(
          `REPLACE: ${patchesPaths(operation.path)}. Value = ${value}`,
        );
      } else if (operation.op == "add") {
        operations.push(
          `ADD: ${patchesPaths(operation.path)}. Value = ${value}`,
        );
      } else if (operation.op == "remove") {
        operations.push(
          `REMOVE: ${patchesPaths(operation.path)}. Value = ${value}`,
        );
      }
    });
...
export function buildMemStatesStrings(
  stores: storeProducePatches,
): memDescriptors {
    ...
    state.mems.forEach((memInst) => {
      if (objectIsEmpty(memInst.data)) {
        ...
        for (let cell in memBuffer) {
          memBufferString = memBufferString.concat(
            `[${cell}] 0x` + memBuffer[cell].toString(16) + " ",
          );
        }
...
export function buildMemStatesArrays(
  stores: storeProducePatches,
): number[][][] {
