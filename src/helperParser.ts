...
import {
  ...
  parseBlockType,
  parseIfBlock,
} from "./instructionsParser";
import { logAsHex } from "./utils";
// import * as assert from "assert";
import { Block } from "typescript";

export function parseidx(bytes: Uint8Array, index: number): [number, number] {
  //thats literally a parse int
  const [id, width] = lebToInt(bytes.slice(index, index + 4));
  return [id, index + width];
}
export function parseTableType(
  bytes: Uint8Array,
  index: number,
): [types.tableType, number] {
  // reftype
  if (bytes[index] != 0x70 && bytes[index] != 0x6f)
    throw new Error("Invalid refType value.");
...
export function parseLimits(
  ...
  // limits flag
  if (bytes[index] != 0x00 && bytes[index] != 0x01)
    throw new Error("Invalid flag in tableType.");
...
export function parseGlobalType(
  bytes: Uint8Array,
  index: number,
): [types.globalType, number] {
...
export function parseValType(
  bytes: Uint8Array,
  index: number,
): [types.valType, number] {
  switch (bytes[index]) {
    case 0x7f:
    case 0x7e:
    case 0x7d:
    ...
    default:
      let error = new Error(
        `Invalid valType at ${index} ${bytes[index].toString(16)}, ${Array.from(
          bytes.slice(index - 5, index + 1),
        ).map((x) => x.toString(16))}`,
      );
      throw error;
  }
...
export function parseBlockExpr(
  bytes: Uint8Array,
  index: number,
  parentBlockType: types.blockType,
): [Op[], number] {
...
export class IfElseOp extends Op {
  constructor(
    public ifBlock: BlockOp,
    public elseBlock: BlockOp | undefined,
    public indexNum = 0,
  ) {
...
export function parseInstruction(
  ...
  if (bytes[index] == op.Opcode.If) {
    ...
    bytes[index] == op.Opcode.Block ||
    bytes[index] == op.Opcode.Loop
  ) {
  ...
  // Memory Instructions
    ...
    bytes[index] == op.Opcode.memorySize ||
    bytes[index] == op.Opcode.memoryGrow
  ) {
...
export function parseCustomNameSection(
  bytes: Uint8Array,
  index: number,
): [types.custom, number] {
  ...
  switch (subsecId) {
    case 0: {
      [names, index] = parseName(bytes, index);
      break;
    }
    case 1: {
      [names, index] = parseFunctionNames(bytes, index);
      break;
    }
    case 2: {
    ...
    case 4: {
      [names, index] = parseTypeNames(bytes, index);
      break;
    }
    case 5: {
      [names, index] = parseTableNames(bytes, index);
      break;
    }
    default: {
      const bytesPosOutput = bytes.slice(index, index + 4);
      let bytesStrOutput: string = "";
      bytesPosOutput.forEach(
        (byte) =>
          (bytesStrOutput = bytesStrOutput.concat(`${byte.toString(16)} `)),
      );
      throw new Error(
        `Unrecognized custom subSection ID (got ${subsecId} at ${bytesStrOutput})`,
      );
    }
  }
...
export function parseLocalNames(
  bytes: Uint8Array,
  index: number,
): [types.indirectNameAssoc[], number] {
...
export function parseTypeNames(
  bytes: Uint8Array,
  index: number,
): [types.nameAssoc[], number] {
...
export function parseTableNames(
  bytes: Uint8Array,
  index: number,
): [types.nameAssoc[], number] {
